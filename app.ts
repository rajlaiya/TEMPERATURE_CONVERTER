...
// Global third-party declarations
declare const Chart: any;
declare const lucide: any;

// --- STATE MANAGEMENT ---
let selectedUnit: 'C' | 'F' = 'C';
let currentWeatherData: any = null;
let activeSeason: 'summer' | 'winter' | 'monsoon' = 'summer';
...
const radarCanvas = document.getElementById('radar-overlay') as HTMLCanvasElement;
const radarCtx = radarCanvas.getContext('2d') as CanvasRenderingContext2D;

let bgParticles: Particle[] = [];
let canvasWidth: number, canvasHeight: number;
let radarWidth: number, radarHeight: number;
let animationId: number;
let radarAngle = 0;
let radarBlips: { x: number; y: number; life: number; size: number }[] = [];
...
class Particle {
    ...
    constructor(type: 'snow' | 'ember' | 'rain' | 'ripple') {
        this.type = type;
        this.reset();
    }
...
function initBgParticles(): void {
    ...
    let type: 'ember' | 'snow' | 'rain' = activeSeason === 'summer' ? 'ember' : (activeSeason === 'winter' ? 'snow' : 'rain');
...
function drawTechBackgroundGrid(): void {
...
function applyCyberTheme(): void {
    ...
    const panels = document.querySelectorAll('.cyber-panel') as NodeListOf<HTMLElement>;
...
function getClosestSatellite(lat: number, lon: number): SatelliteDetails {
    if (lon >= -30 && lon < 30) {
...
async function fetchWeatherData(lat: number, lon: number, cityName: string): Promise<void> {
    logInference(`[SYS] Uplinking weather forecast data for coordinates: [${lat.toFixed(4)}N, ${lon.toFixed(4)}E]`);
    
    const statusBadge = document.getElementById('ai-status-text') as HTMLElement;
...
function updateTempGauge(): void {
    const tempValEl = document.getElementById('temp-val') as HTMLElement;
...
function interpretWeatherCode(code: number): { desc: string; icon: string } {
...
function detectSeasonFromWeather(data: any): void {
    const temp = data.current.temperature_2m;
    const rain = data.current.precipitation || 0;
    const month = new Date().getMonth();
    const lat = data.latitude;

    let detected: 'summer' | 'monsoon' | 'winter' = 'summer';
...
function estimateSeasonalAverages(lat: number, currentC: number): void {
...
function render10DayTimeline(data: any): void {
    const listEl = document.getElementById('forecast-10day-list') as HTMLElement;
    listEl.innerHTML = '';
...
function expandDayDetails(index: number): void {
    if (!currentWeatherData) return;

    const detailsPanel = document.getElementById('forecast-day-details') as HTMLElement;
    ...
    const rainProb = currentWeatherData.daily.precipitation_probability_max[index] || 0;
...
function updateTemperatureConverter(celsiusTemp: number): void {
...
function logInference(msg: string): void {
    const consoleBox = document.getElementById('ai-log') as HTMLElement;
...
function simulateAIPrediction(data: any): void {
...
// Render Chart.js
function renderForecastChart(data: any): void {
    ...
    const ctxChart = chartCanvas.getContext('2d') as CanvasRenderingContext2D;
...
function generateAIAdvisory(data: any): void {
    const advText = document.getElementById('ai-advisory') as HTMLElement;
...
const suggestionsBox = document.getElementById('suggestions') as HTMLElement;
...
function attemptSatelliteGeolocation(): void {
    const loaderStatusText = document.getElementById('loader-status-text') as HTMLElement;
...
// --- SHARE HANDLER ---
function shareApp(e: Event): void {
    e.preventDefault();
...
(window as any).shareApp = shareApp;
(window as any).expandDayDetails = expandDayDetails;
